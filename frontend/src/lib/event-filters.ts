...
export interface EventFilters {
  ...
  // Distance range filtering (0-30km, 30 means no upper limit)
  maxDistance: number
...
export const createDefaultFilters = (): EventFilters => ({
  eventTypes: [],
...
// Filter update helper type
export type FilterUpdate = Partial<EventFilters>
    ...
    if (filters.dateRange !== 'all' && event.event_start_date) {
      ...
      switch (filters.dateRange) {
        ...
        case 'quarter': {
          ...
          if (!(eventDate >= now && eventDate <= quarterFromNow)) return false
          break
        }
        case 'custom': {
          if (filters.customDateStart && eventDate < filters.customDateStart) return false
          if (filters.customDateEnd && eventDate > filters.customDateEnd) return false
          break
        }
      }
    }
...
export const filterHelpers = {
  ...
  toggleArrayFilter: <T>(array: T[], item: T): T[] => {
  ...
  // Clear all filters
  clearAllFilters: (): EventFilters => createDefaultFilters(),
  ...
  getActiveFilterCount: (filters: EventFilters): number => {
    let count = 0
    if (filters.eventTypes.length > 0) count++
    if (filters.maxDistance < 30) count++ // 30 is "no limit"
    ...
    async function loadEvents() {
      try {
        ...
        const futureEvents = validatedEvents.filter(event => {
          if (!event.event_start_date) return true // Keep events without dates
          const eventDate = new Date(event.event_start_date)
...
export function useEventFilters(initialFilters?: Partial<EventFilters>) {
  ...
  const updateFilters = useCallback((updates: FilterUpdate) => {
