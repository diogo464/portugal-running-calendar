...
def fetch_event_pages():
    """Fetch all events from WordPress API across multiple pages until empty response."""
    events = []
    page = 1
...
def geocode_location(location: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Parse the JSON output from the geocoding script - look for the final JSON block
        ...
        for i, line in enumerate(output_lines):
            if line.strip().startswith('{') and 'coordinates' in result.stdout[result.stdout.find(line):]:
                json_start_idx = i
            if json_start_idx != -1 and line.strip().endswith('}'):
                json_end_idx = i
                break
        
        if json_start_idx != -1 and json_end_idx != -1:
            json_content = '\n'.join(output_lines[json_start_idx:json_end_idx+1])
            try:
                ...
                print(f"JSON decode error for location '{location}': {e}")
                return None
...
def main():
    print("Starting comprehensive event data extraction...")
    
    # Fetch all events
    print("Fetching all events from WordPress API...")
    all_events = fetch_event_pages()
    ...
    for i, event in enumerate(all_events):
        try:
            if i % 10 == 0:
                print(f"Progress: {i}/{len(all_events)} events processed")
            ...
            error_msg = f"Error processing event {event.get('id', 'unknown')}: {e}"
            print(error_msg)
    ...
    print(f"\nCompleted! Processed {len(processed_events)} events")
